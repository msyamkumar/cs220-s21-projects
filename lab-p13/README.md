...
*TA: split students into breakout room for 5-10 minutes. After returning from the breakout rooms, fill above ???? with inputs from the students.*
...
*TA: start a poll to ask for feedback from students (eg. wait for a few minutes, re-explain the topic or continue).*
...
matrix[????][????]
```
*TA: randomly ask one student to fill ???? above.*
